---
layout: ../layouts/LearnLayout.astro
title: "Learn"
---
# Introduction
RAID is an objective-based third-person shooter created in Fortnite. 

Two opposing teams, the "Defender" and the "Raider," compete against each other. 

The Raider's objective is to plant a bomb, while the Defender aims to stop them. Players acquire weapons through a weapon shop UI that appears at the start of the game. 

There are 51 selected weapons and items in RAID. RAID features Kill Achievement and Ranking System.

# Game-play and general rules
- Each match has 15 rounds. Note: Each match is supposed to have 30 rounds, and teams will be rotated after 15 rounds. However, the Team Rotation setting is not working right now, so I have to disable It. It will be updated when Epic Games fixes this issue in the future.
- Each round lasts 150 seconds, plus 45 seconds if the Raider plants the bomb.
- Raider's objectives: Plant the bomb at bombsite A or B. Or eliminate all Defenders before time out.
- Defender's objectives: Defends bombsites A and B, defuse the bomb if the Raider plants it, or eliminate all the Raiders.
- It takes 4 seconds to plant or defuse the bomb.
<video width="1280" height="720" controls src="/videos/bomb-planted.webm"></video>

- Each team receives 1000 gold each round, and the winning team gets an extra 2000 gold. The player receives 300 gold for each elimination.
- The player acquires weapons through the weapon shop UI that appears at the start of the game.
- The player has 30 seconds to buy the weapon before the weapon shop automatically closes. However, the player can close the weapon shop UI after 5 seconds.
- Once the weapon shop UI closes. It will not show up again until the next round.

<video width="1280" height="720" controls src="/videos/weapon-shop-ui.webm"></video>

# Kill achievements system
- RAID tracks the following kill achievements: Headshot, Singlekill, Doublekill, Triplekill, Quadrakill, and Pentakill. 
- To achieve multi-kill achievements, the player must eliminate an opponent within 4 seconds.

# Ranking system
  There are total 11 ranks tier in RAID:
- Unranked
- Iron I, Iron II, Iron III, Iron IV, Iron V
- Bronze I, Bronze II, Bronze III, Bronze IV, Bronze V
- Silver I, Silver II, Silver III, Silver IV, Silver V
- Gold I, Gold II, Gold III, Gold IV, Gold V
- Platinum I, Platinum II, Platinum III, Platinum IV, Platinum V
- Diamond I, Diamond II, Diamond III, Diamond IV, Diamond V
- Elite I, Elite II, Elite III, Elite IV, Elite V
- Master I, Master II, Master III, Master IV, Master V
- Grandmaster I, Grandmaster II, Grandmaster III, Grandmaster IV, Grandmaster V
- Legendary I, Legendary II, Legendary III, Legendary IV, Legendary V

# Accolades XP Awarding
- The player is awarded XP when eliminating an opponent, planting a bomb, or defusing a bomb, and the bomb is denoted (for the planted bomb player)

# Weapons
Visit [Weapons](/weapons) page to see all available weapons and items in RAID

# RAID: GHOST Game-play overview
> Work in progress. Please Stay in touch!





